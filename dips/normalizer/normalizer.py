...
class Normalizer:
    """
    Normalizer for Parallel Bar Dips.
    ...
    def __init__(self):
        ...
        # We use the SHOULDER as the origin for the Dip
    ...
    def _normalize_point(self, x, y, z, origin_x, origin_y, origin_z, facing, scale):
        ...
        # Result: User faces RIGHT (+X). 
        norm_x = ((x - origin_x) * facing) / scale
        
        # Y: Shift origin to Shoulder, Invert (Up is +Y), Scale by Torso Length
        # Result: Dropping into the dip moves the wrist UP (+Y) relative to the shoulder.
    ...
    def process(self, landmarks, calibration_data=None):
        ...
        if calibration_data:
            ...
            # Gatekeeper passes torso length as scale_factor
