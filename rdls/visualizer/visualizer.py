...
class RdlsVisualizer:
    ...
    def draw(self, frame, packet):
        """
        Main rendering method for RDL analysis.
        """
        if packet is None:
            return frame
    ...
    def _draw_skeleton(self, frame, landmarks, faults, active_side):
        ...
        connections = [
            (sh_idx, hip_idx),  # Torso
            (hip_idx, knee_idx), # Thigh
            (knee_idx, ank_idx), # Shin
            (sh_idx, wr_idx)   # Arm holding weight
        ]
        ...
        # Draw Joints
        joint_indices = [sh_idx, hip_idx, knee_idx, ank_idx, wr_idx]
        for idx in joint_indices:
            ...
            if lm.visibility > 0.5:
                ...
                if "SQUAT_FAULT" in faults and idx == knee_idx:
                    color = self.COLORS['RED']
                elif "LUMBAR_ROUND" in faults and idx == hip_idx:
                    color = self.COLORS['RED']
                elif "BAR_DRIFT" in faults and idx == wr_idx:
                    color = self.COLORS['RED']
