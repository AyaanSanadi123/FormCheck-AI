...
class PushUpsNormalizer:
    ...
    def process(self, landmarks, calibration_data):
        ...
        # 1. Identify Anchor (Hip)
        origin_x, origin_y = landmarks[hip_idx].x, landmarks[hip_idx].y

        # 2. Calculate Plank Rotation
        dx_raw = landmarks[sh_idx].x - landmarks[hip_idx].x
        dy_raw = landmarks[sh_idx].y - landmarks[hip_idx].y
        
        # Angle to make the Shoulder-Hip vector horizontal
