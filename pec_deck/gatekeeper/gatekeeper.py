...
class PecDeckGatekeeper:
    def __init__(self):
        ...
        self.FRONTAL_Z_TOLERANCE = 0.15 # Shoulders should be equidistant
        self.SHOULDER_LEVEL_TOLERANCE = 0.05 # Shoulders should be level
    ...
    def check(self, landmarks):
        ...
        # Critical for Pec Deck: Shoulders, Elbows, Wrists, Hips
        critical_indices = [
        ...
        # User must be facing the camera. Check Z-depth of shoulders.
        z_diff = abs(landmarks[self.MP_POSE.LEFT_SHOULDER.value].z - landmarks[self.MP_POSE.RIGHT_SHOULDER.value].z)
        if z_diff > self.FRONTAL_Z_TOLERANCE:
            self._reset("User not frontal")
            return False, "Face the camera directly", None
        ...
        if shoulder_level > self.SHOULDER_LEVEL_TOLERANCE:
            ...
            return False, "Sit straight, keep shoulders level", None

        # --- STEP 4: TEMPORAL BUFFERING ---
        # Track shoulder width for stability and calibration
        ...
        frame_metrics = {
            ...
            "mid_shoulder_y": (landmarks[self.MP_POSE.LEFT_SHOULDER.value].y + landmarks[self.MP_POSE.RIGHT_SHOULDER.value].y) / 2,
            "timestamp": time.time()
        }
    ...
    def _generate_passport(self):
        ...
        avg_y = np.mean([m['mid_shoulder_y'] for m in self.validation_buffer])
